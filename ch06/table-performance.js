...
const main = () => {
    ...
    assert(someLabels.length > 0, "Must be some labels for select (array to short)")

    const [rowTable, rowSize, rowHeap] = memory(buildRows, nRows, labels)
    const [colTable, colSize, colHeap] = memory(buildCols, nCols, labels)

    const rowFilterTime = time(rowFilter, rowTable,
        row => row.label_1 % RANGE === 0)
    const rowSelectTime = time(rowSelect, rowTable, someLabels)

    const colFilterTime = time(colFilter, colTable,
...
const colFilter = (table, func) => {
    const result = {}
    const labels = Object.keys(result) // table?
